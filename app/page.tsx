...
function Badge({ children, on = "light" }: { children: React.ReactNode; on?: "light" | "warm" | "dark" }) {
  ...
  return <span className={`inline-block rounded-full px-4 py-1.5 text-sm font-700 ${cls}`}>{children}</span>;
}
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 bg-canvas/80 backdrop-blur-md">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            <Link href="/login" className="hidden rounded-full px-4 py-2.5 text-sm font-700 text-ink transition-colors hover:bg-white sm:block">Ingresar</Link>
            <a href="#contacto" className="rounded-full bg-cta px-5 py-2.5 text-sm font-700 text-white shadow-soft transition-colors hover:bg-cta-deep">Solicitar demo</a>
          </div>
        </nav>
      </header>
      ...
      <section className="relative overflow-hidden">
        <Loops className="right-[46%] top-4 h-16 w-28 text-brand/25" />
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 pb-14 pt-12 lg:grid-cols-2 lg:pb-24 lg:pt-16">
          <div className="relative z-10">
            <Badge>Plataforma de comunicación escolar</Badge>
            <h1 className="mt-6 font-display text-4xl font-700 leading-[1.1] text-ink sm:text-5xl lg:text-[3.4rem]">
              <Mark>Conecta</Mark> a tu colegio con las familias <Blue>como nunca</Blue>
            </h1>
            <p className="mt-5 max-w-lg text-lg font-400 leading-relaxed text-ink/65">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a href="#contacto" className="flex items-center justify-center gap-2 rounded-full bg-cta px-7 py-4 text-base font-700 text-white shadow-soft transition-colors hover:bg-cta-deep">
                Empieza hoy <Icon name="ArrowRight" className="h-5 w-5" />
              </a>
              <a href="#funciones" className="flex items-center justify-center gap-2 rounded-full border-2 border-ink/10 bg-white px-7 py-4 text-base font-700 text-ink transition-colors hover:border-brand/40 hover:text-brand">
                Ver funciones
              </a>
            </div>
            <dl className="mt-10 grid max-w-md grid-cols-2 gap-6">
              <div><dt className="font-display text-3xl font-700 text-ink">+120</dt><dd className="text-sm font-500 text-ink/55">Colegios conectados</dd></div>
              <div><dt className="font-display text-3xl font-700 text-ink">98%</dt><dd className="text-sm font-500 text-ink/55">Avisos leídos</dd></div>
            </dl>
          </div>

          <div className="relative">
            <Ring className="-right-3 -top-3 h-16 w-16 text-cta/25" />
            <DottedArc className="-bottom-6 -left-5 h-20 w-20 text-brand/30" />
            ...
            <div className="absolute left-3 top-6 z-10 flex items-center gap-2.5 rounded-3xl border border-ink/5 bg-white p-3 shadow-card">
              <span className="relative grid h-12 w-12 place-items-center">
                <ProgressRing value={98} className="h-12 w-12" />
                <span className="absolute font-display text-[11px] font-700 text-ink">98%</span>
              </span>
              <span className="text-xs font-600 leading-tight text-ink">Avisos<br />leídos</span>
            </div>
            <div className="absolute -bottom-3 right-3 z-10 flex items-center gap-3 rounded-3xl border border-ink/5 bg-white p-3 shadow-card">
              ...
              <span className="text-xs font-600 leading-tight text-ink">+30,000<br />familias</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-6">
        <p className="text-center text-sm font-600 text-ink/45">Colegios de toda Latinoamérica confían en Colequium</p>
        <div className="mt-6 flex flex-wrap items-center justify-center gap-x-10 gap-y-4">
          {["Colegio Las Lomas", "Instituto del Sol", "Colegio San Marcos", "Liceo Norte", "Escuela Río"].map((n) => (
            <span key={n} className="font-display text-lg font-700 text-ink/30">{n}</span>
          ))}
        </div>
      </section>

      {/* ===== Funciones (fondo claro) ===== */}
      <section id="funciones" className="relative overflow-hidden py-16 sm:py-24">
        <Dots className="right-8 top-10 h-16 w-16 text-cta/30" />
        <div className="relative z-10 mx-auto max-w-6xl px-5">
          <div className="mx-auto max-w-xl text-center">
            ...
            <p className="mt-3 font-400 text-ink/65">Seis módulos que reemplazan los grupos de chat, los papeles y los mails perdidos.</p>
          </div>
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FUNCIONES.map((f) => (
              <div key={f.title} className={`group rounded-[1.75rem] p-6 shadow-card transition-transform hover:-translate-y-1 ${f.featured ? "bg-ink text-white" : "border border-ink/5 bg-white"}`}>
                <span className={`grid h-14 w-14 place-items-center rounded-2xl ${f.featured ? "bg-cta text-white" : ACCENT_ON[f.color]}`}>
                  <Icon name={f.icon} className="h-7 w-7" />
                </span>
                <h3 className={`mt-5 font-display text-xl font-700 ${f.featured ? "text-white" : "text-ink"}`}>{f.title}</h3>
                <p className={`mt-2 text-sm font-400 leading-relaxed ${f.featured ? "text-white/70" : "text-ink/60"}`}>{f.text}</p>
      ...
      <section className="relative overflow-hidden py-16 sm:py-24">
        <div className="relative z-10 mx-auto grid max-w-6xl items-center gap-12 px-5 lg:grid-cols-2">
          <div className="relative">
            <Sparkle className="-left-4 -top-4 -z-10 h-10 w-10 text-cta/80" />
            <Dots className="-right-3 -bottom-3 -z-10 h-16 w-16 text-brand/25" />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <div className="absolute -bottom-5 -right-3 rounded-3xl border border-ink/5 bg-white p-4 shadow-pop">
              <p className="font-display text-2xl font-700 text-brand">+5 años</p>
              <p className="text-xs font-500 text-ink/55">acompañando colegios</p>
            </div>
          </div>
          <div>
            ...
            <p className="mt-4 font-400 text-ink/65">
            ...
            <ul className="mt-6 flex flex-col gap-3">
              {["Una sola app para avisos, calendario, mensajes y trámites", "Pensado para directivos, docentes y familias", "Listo para crecer a varios países e idiomas"].map((t) => (
                <li key={t} className="flex items-start gap-3 text-sm font-500 text-ink/75">
      ...
      {/* ===== Valores ===== */}
      <section className="relative overflow-hidden py-16 sm:py-24">
        <div className="relative z-10 mx-auto grid max-w-6xl gap-12 px-5 lg:grid-cols-2 lg:items-center">
          <div className="relative">
            <Squiggle className="-top-8 left-1 h-10 w-24 text-cta/50" />
            <Badge>Nuestros valores</Badge>
            <h2 className="mt-4 font-display text-3xl font-700 leading-tight text-ink sm:text-4xl">En qué <Blue>creemos</Blue></h2>
            <p className="mt-4 max-w-md font-400 text-ink/65">Cada decisión de producto pasa por estos cuatro principios.</p>
          </div>
          <div className="grid gap-x-6 gap-y-8 sm:grid-cols-2">
            {VALORES.map((v) => (
              <div key={v.title}>
                ...
                <h3 className="mt-4 font-display text-lg font-700 text-ink">{v.title}</h3>
                <p className="mt-1.5 text-sm font-400 leading-relaxed text-ink/60">{v.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ===== Para cada rol (bloque flotante) ===== */}
      <section className="py-8 sm:py-12">
        <div className="mx-auto max-w-6xl px-5">
          <div className="relative overflow-hidden rounded-[2.5rem] bg-warm p-8 sm:p-14">
            <Loops className="right-8 top-8 h-16 w-28 text-white/45" />
            <div className="relative z-10">
              <div className="mx-auto max-w-xl text-center">
                <Badge on="warm">Para cada rol</Badge>
                <h2 className="mt-4 font-display text-3xl font-700 text-ink sm:text-4xl">Pensado para <Blue>toda la comunidad</Blue></h2>
              </div>
              <div className="mt-10 flex flex-col gap-4">
                {ROLES.map((r) => (
                  <div key={r.title} className="grid gap-5 overflow-hidden rounded-[1.5rem] bg-white p-5 shadow-card sm:grid-cols-[1.4fr_1fr] sm:items-center">
                    <div className="order-2 sm:order-1">
                      <span className={`inline-block rounded-full px-3 py-1 text-xs font-700 ${ACCENT_ON[r.color]}`}>{r.tag}</span>
                      <h3 className="mt-3 font-display text-xl font-700 text-ink">{r.title}</h3>
                      <p className="mt-2 text-sm font-400 leading-relaxed text-ink/60">{r.text}</p>
                      <div className="mt-4 flex flex-wrap gap-2">
                        {r.meta.map((m) => (
                          <span key={m} className="inline-flex items-center gap-1.5 rounded-full bg-mist px-3 py-1 text-xs font-600 text-ink/60">
                            <Icon name="Check" className="h-3 w-3 text-brand" />{m}
                          </span>
                        ))}
                      </div>
                      <a href="#contacto" className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-ink px-4 py-2 text-sm font-700 text-white transition-colors hover:bg-navy-deep">
      ...
      <section id="planes" className="relative overflow-hidden py-16 sm:py-24">
        <Dots className="right-10 top-12 h-14 w-14 text-cta/30" />
        <div className="relative z-10 mx-auto max-w-6xl px-5">
          <div className="mx-auto max-w-xl text-center">
            <Badge>Planes</Badge>
            <h2 className="mt-4 font-display text-3xl font-700 text-ink sm:text-4xl">El plan ideal <Blue>para tu colegio</Blue></h2>
          ...
          <div className="mt-12 grid items-start gap-5 lg:grid-cols-3">
            {PLANES.map((p) => (
              <div key={p.name} className={`relative rounded-[1.75rem] p-7 shadow-card ${p.featured ? "bg-gradient-to-br from-navy to-navy-deep text-white lg:-translate-y-3" : "border border-ink/5 bg-white"}`}>
                {p.featured && <span className="absolute right-6 top-7 inline-flex items-center gap-1.5 rounded-full bg-cta px-3 py-1 text-xs font-700 text-white"><Icon name="Star" className="h-3.5 w-3.5 fill-white" /> Más elegido</span>}
                <span className={`text-sm font-700 ${p.featured ? "text-white/70" : "text-ink/45"}`}>{p.name}</span>
                <div className="mt-3 flex items-end gap-1">
                  <span className={`font-display text-4xl font-700 ${p.featured ? "text-white" : "text-ink"}`}>{p.price}</span>
                  <span className={`mb-1 text-sm font-500 ${p.featured ? "text-white/60" : "text-ink/45"}`}>/ {p.period}</span>
                </div>
                <p className={`mt-3 text-sm font-400 ${p.featured ? "text-white/70" : "text-ink/55"}`}>{p.desc}</p>
                <ul className="mt-6 flex flex-col gap-3">
                  {p.features.map((f) => (
                    <li key={f} className={`flex items-center gap-2.5 text-sm font-500 ${p.featured ? "text-white/85" : "text-ink/70"}`}>
                ...
                <a href="#contacto" className={`mt-7 block rounded-full py-3.5 text-center text-sm font-700 transition-colors ${p.featured ? "bg-cta text-white hover:bg-cta-deep" : "bg-ink text-white hover:bg-navy-deep"}`}>Solicitar demo</a>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative overflow-hidden py-16 sm:py-24">
        <div className="relative z-10 mx-auto max-w-6xl px-5">
          <div className="mx-auto max-w-xl text-center">
            ...
            <h2 className="mt-4 font-display text-3xl font-700 text-ink sm:text-4xl">Tu colegio conectado <Blue>en 4 pasos</Blue></h2>
          </div>
            ...
            {PASOS.map((s, i) => (
              <div key={s.title} className="relative text-center">
                <div className="relative mx-auto grid h-16 w-16 place-items-center rounded-[1.4rem] bg-white text-brand shadow-card ring-1 ring-ink/5">
                  ...
                  <span className="absolute -right-1 -top-1 grid h-6 w-6 place-items-center rounded-full bg-cta font-display text-xs font-700 text-white">{i + 1}</span>
                </div>
                <h3 className="mt-4 font-display text-lg font-700 text-ink">{s.title}</h3>
                <p className="mt-1.5 text-sm font-400 leading-relaxed text-ink/60">{s.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ===== Testimonio (bloque flotante) ===== */}
      <section className="py-8 sm:py-12">
        <div className="mx-auto max-w-5xl px-5">
          <div className="relative overflow-hidden rounded-[2.5rem] bg-warm p-8 text-center sm:p-14">
            <Loops className="left-8 top-8 h-16 w-28 text-white/45" />
            <Sparkle className="right-10 bottom-10 h-8 w-8 text-ink/20" />
            <div className="relative z-10 mx-auto max-w-2xl">
              <Badge on="warm">Testimonios</Badge>
              <h2 className="mt-4 font-display text-3xl font-700 text-ink sm:text-4xl">Lo que dicen <Blue>los colegios</Blue></h2>
              <div className="relative mt-8 rounded-[1.75rem] bg-white p-8 shadow-pop sm:p-10">
                <Icon name="Quote" className="mx-auto h-9 w-9 text-cta/30" />
                ...
                <p className="mt-5 text-lg font-500 leading-relaxed text-ink/80">
                ...
                <div className="mt-6 flex items-center justify-center gap-3">
                  ...
                  <div className="text-left">
                    <p className="font-display text-sm font-700 text-ink">Mariana Saldaña</p>
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-5">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-4 font-display text-3xl font-700 text-ink sm:text-4xl">Ideas para <Blue>tu comunidad</Blue></h2>
            </div>
            <a href="#" className="rounded-full border-2 border-ink/10 bg-white px-5 py-2.5 text-sm font-700 text-ink transition-colors hover:border-brand/40 hover:text-brand">Ver todo</a>
          </div>
          <div className="mt-10 grid gap-5 lg:grid-cols-2">
            <article className="overflow-hidden rounded-[1.75rem] border border-ink/5 bg-white shadow-card transition-transform hover:-translate-y-1">
              ...
              <div className="p-6">
                <span className="text-xs font-700 text-cta">{RECURSOS[0].tag}</span>
                <h3 className="mt-2 font-display text-xl font-700 leading-snug text-ink">{RECURSOS[0].title}</h3>
                <p className="mt-2 text-xs font-500 text-ink/45">{RECURSOS[0].date}</p>
              </div>
            </article>
              ...
              {RECURSOS.slice(1).map((r) => (
                <article key={r.title} className="flex gap-4 overflow-hidden rounded-[1.75rem] border border-ink/5 bg-white p-3 shadow-card transition-transform hover:-translate-y-1">
                  ...
                  <div className="py-2 pr-2">
                    <span className="text-xs font-700 text-cta">{r.tag}</span>
                    <h3 className="mt-1 font-display text-base font-700 leading-snug text-ink">{r.title}</h3>
                    <p className="mt-2 text-xs font-500 text-ink/45">{r.date}</p>
                  </div>
                </article>
              ))}
            </div>
      ...
      {/* ===== FAQ ===== */}
      <section className="relative overflow-hidden py-16 sm:py-24">
        <div className="relative z-10 mx-auto grid max-w-6xl gap-10 px-5 lg:grid-cols-[0.8fr_1.2fr]">
          <div className="relative">
            <Sparkle className="-top-6 right-10 -z-10 h-9 w-9 text-cta/70" />
            <Badge>Preguntas frecuentes</Badge>
            <h2 className="mt-4 font-display text-3xl font-700 leading-tight text-ink sm:text-4xl">¿Tienes <Blue>dudas?</Blue></h2>
            <p className="mt-4 font-400 text-ink/60">Resolvemos las preguntas más comunes. ¿Tienes otra? Escríbenos y te respondemos enseguida.</p>
          </div>
          <div className="flex flex-col gap-3">
            {FAQS.map((f) => (
              <details key={f.q} className="group rounded-2xl border border-ink/10 bg-white p-5 [&_summary::-webkit-details-marker]:hidden">
                <summary className="flex cursor-pointer items-center justify-between gap-4 font-display text-base font-700 text-ink">
                  {f.q}
      ...
      {/* ===== CTA + Contacto (bloque flotante) ===== */}
      <section id="contacto" className="mx-auto max-w-6xl px-5 pb-20 pt-4">
        <div className="relative overflow-hidden rounded-[2.5rem] bg-warm p-8 text-center shadow-pop sm:p-14">
          <Loops className="left-6 top-8 h-16 w-28 text-white/50" />
          <Dots className="right-8 bottom-8 h-16 w-16 text-ink/15" />
          ...
          <div className="relative z-10 mx-auto max-w-xl">
            <span className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-1.5 text-sm font-700 text-cta"><span className="h-2 w-2 rounded-full bg-cta" /> Comunicación escolar</span>
            <h2 className="mt-5 font-display text-3xl font-700 text-ink sm:text-4xl">¿Listo para conectar tu colegio?</h2>
            <p className="mx-auto mt-3 max-w-md font-400 text-ink/70">Déjanos tus datos y coordinamos una demo. El acceso de familias y docentes es por invitación del colegio.</p>
            <form className="mx-auto mt-7 flex max-w-md flex-col gap-3 sm:flex-row">
              <input type="email" required placeholder="Correo del colegio" className="min-w-0 flex-1 rounded-full bg-white px-5 py-3.5 text-sm font-500 text-ink outline-none placeholder:text-ink/40 focus:ring-2 focus:ring-ink/20" />
              <button type="submit" className="rounded-full bg-ink px-6 py-3.5 text-sm font-700 text-white shadow-card transition-colors hover:bg-navy-deep">Solicitar demo</button>
            </form>
          </div>
        </div>
      </section>
      ...
      <footer className="border-t border-ink/5 bg-white">
        <div className="mx-auto grid max-w-6xl gap-10 px-5 py-14 sm:grid-cols-2 lg:grid-cols-4">
          <div className="sm:col-span-2 lg:col-span-1">
            <Wordmark />
            <p className="mt-4 max-w-xs text-sm font-400 text-ink/55">La comunidad escolar, conectada en un solo lugar. Para LatAm y Brasil.</p>
          </div>
          {[
            ...
            <div key={col.h}>
              <h3 className="font-display text-sm font-700 text-ink">{col.h}</h3>
              <ul className="mt-4 flex flex-col gap-2.5 text-sm font-400 text-ink/55">
        ...
        <div className="border-t border-ink/5">
          <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 px-5 py-6 text-sm font-400 text-ink/45 sm:flex-row">
